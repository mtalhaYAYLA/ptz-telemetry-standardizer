...
# Zeep (SOAP/WSDL) kütüphanesi karmaşık XML şemalarını parse ederken
# Python'un varsayılan 1000 frame limitini aşabiliyor.
...
def setup_database():
    ...
    # WAL modu: okuyucular yazarları, yazarlar okuyucuları bloklamaz.
    # 4 kamera thread'i + dispatcher aynı anda DB'ye eriştiğinde kilit sorununu önler.
...
class CameraMonitor():
    ...
    def __init__(self, config, anomaly_logger, ram_queue, priorities=None, usecases=None):
        ...
        self._last_pan = None   # reconnect sonrası kamerayı geri götürmek için
    ...
    def _connect_onvif(self):
        # Eski zeep/WSDL state'ini serbest bırak; stale nesneler recursion'a yol açabilir.
        self.onvif_cam = None
        self.ptz = None
        try:
            ...
            self.status_message = "ONVIF Bağlantı Hatası: Recursion limit aşıldı (zeep/WSDL)"
    ...
    def _go_to_degree(self, pan_deg, tilt_deg):
        ...
        try:
            ...
            self._last_pan = pan_deg  # başarılı hareketi kaydet
    ...
    def _check_onvif_connection(self):
        if not self.onvif_cam:
            ok = self._connect_onvif()
            if ok and self._last_pan is not None:
                self._go_to_degree(self._last_pan, self._last_tilt)
            return ok
        try:
            ...
            ok = self._connect_onvif()
            if ok and self._last_pan is not None:
                # Kamera firmware'i yeni session'da home pozisyonuna gidebilir,
                # son bilinen konuma geri gönder.
                logging.info(f"[{self.name}] Reconnect sonrası son konuma dönülüyor: Pan={self._last_pan}°, Tilt={self._last_tilt}°")
                self._go_to_degree(self._last_pan, self._last_tilt)
            return ok
    ...
    def run(self):
        # Dış while: exception sonrası thread ölmez, stack temiz şekilde yeniden başlar.
        # self.run() recursive çağrısı yerine bu döngü kullanılır.
        while self._run_flag:
            try:
                if not self._connect_onvif():
                    self.status_message = "ONVIF bağlantısı kurulamadı, tarama durdu."
                ...
                if pan_start == pan_end: # Sabit pozisyon senaryosu
                    ...
                    while self._run_flag:
                        ...
                        time.sleep(1)
                    break  # _run_flag False olduğunda dış döngüden çık
                ...
                if not full_tour_points:
                    logging.error(f"[{self.name}] Tarama rotası oluşturulamadı."); break
                ...
                logging.critical(f"[{self.name}] TARAMA THREAD'İNDE KRİTİK HATA: {e}", exc_info=True)
                if self._run_flag:
                    logging.info(f"[{self.name}] 30 saniye sonra yeniden başlatılacak...")
                    time.sleep(30)
                    # Dış while döngüsü devam eder — stack temiz, yeni bir frame eklenmez.
